...
보통은 `map`과 `set`이 검색에 유리하지만, 연속 메모리를 가진 `vector`도 데이터 갯수가 적으면 속도가 빠릅니다. 요소 갯수에 따라 가변적이므로 기본적으로 `vector`로 사용하고, 속도가 느린 경우 컨테이너를 이것 저것 바꾼뒤 프로파일러로 속도 측정후 결정하는게 좋습니다. 따라서 코딩시 컨테이너 변경이 용이하도록 작성하여야 합니다.
