...
> * bool 형변환 정의는 하지 마라. 암시적 형변환을 하는 형변환 정의를 하지마라.
> * 할 수 있는한 최선을 다하여 형변환 하지 마라.

**암시적 형변환**

각 자료형은 하기 규칙에 따라 암시적으로 형변환 됩니다. 하지만 명시적으로 변환하는 것이 추후 코드분석에 도움이 됩니다. C언어의 나쁜 잔재이니 사용하지 마세요.
...
암시적으로 형변환 되지 않는다면, 하기와 같이 괄호를 이용하여 명시적으로 형변환 할 수 있습니다.(잘못 사용하면 예기지 못한 데이터 절삭이나 잘못된 변환으로 예외가 발생할 수 있으니므로, 사용하지 않아야 합니다. C언어의 나쁜 잔재입니다.)
...
`static_cast`를 이용하여 암시적으로 형변환되지 않는 것들을 명시적으로 할 수 있습니다.
...
Derived d;
Base* b = &d;
// (O) dynamic_cast는 상속관계가 있어야 함(런타임 검사를 수행함)
...
`reinterpret_cast`는 데이터의 크기나 상속관계를 무시하고 그냥 변환해 버립니다. 하지만, 상수성은 지켜 줍니다. 아무튼 타입에 기반한 **코딩 계약** 을 위반하니 사용하지 마세요.
...
하기와 같이 [캡슐화](https://tango1202.github.io/principle/principle-encapsulation/)를 위해 사용자 정의 형변환을 정의하여, 암시적으로 형변환 되게 할 수 있습니다. 
...
하지만, 기본 암시적 형변환과 잘 호환되도록 주의해서 작성해야 하고, 코드 분석이 모호해 질 수 있으므로 사용하지 마세요. 하기와 같이 멤버함수를 작성해도 충분히 사용하기 좋고, 검색하기 좋고, 분석하기 좋습니다.
...
`bool`은 암시적으로 `int`로 형변환되면서 뜻하지 않은 동작을 할 수 있으므로, `bool` 형변환 정의를 하지 마세요.
