...
`std::unique_ptr`은 소유권을 이전합니다만, `std::shared_ptr`은 개체의 소유권을 공유하는 스마트 포인터입니다. 소유권이 공유될때마다 내부적으로는 참조 카운트를 증가시키고, 스마트 포인터가 소멸될때마다 참조 카운트를 감소시켜 `0`이 될때 `delete`합니다.
...
`std::shared_ptr`은 내부적으로 제어 블록을 별도로 할당하며, 다음 정보를 관리합니다.

1. 관리 대상 개체 포인터
2. `std::shared_ptr` 참조 카운트
3. `std::weak_ptr` 참조 카운트(하기 `std::weak_ptr` 참고)
...
# std::shared_ptr 멤버 함수 

|항목|내용|
|--|--|
`constexpr shared_ptr() noexcept;`<br/><br/>`explicit shared_ptr(T* p);`<br/>`shared_ptr(T* p, deleter);`<br/>`shared_ptr(T* p, deleter, allocator);`<br/><br/>`constexpr shared_ptr(std::nullptr_t) noexcept;`<br/>`shared_ptr(std::nullptr_t p, deleter);`<br/>`shared_ptr(std::nullptr_t p, deleter, allocator);`<br/><br/>`explicit shared_ptr(const std::weak_ptr&);`<br/>`shared_ptr(std::unique_ptr&&);`<br/><br/>별칭 생성자<br/>`shared_ptr(const shared_ptr& other, element_type* p) noexcept;`<br/><br/>`shared_ptr( std::auto_ptr&&);` (C++11~C++17)|`nullptr`이나 `p`를 공유하며, 참조 카운트를 증가시킵니다. 이때 사용자 정의 `deleter`와 `allocator` 를 사용할 수 있습니다. `std::weak_ptr`과 `std::unique_ptr`로 생성할 수도 있습니다.|
...
|`operator [](std::ptrdiff_t) const;` (C++17~)|배열을 관리하는 경우 각 요소 개체의 참조자를 리턴합니다.|
...
|`unique() const noexcept;`(C++11~C++17)|다른 `std::shared_ptr`로 관리하는지 검사합니다. `use_count() == 1`과 같습니다.|
|`lock() const noexcept;`|관리하는 개체 접근을 위해 임시 `std::shared_ptr`을 생성합니다.|
...
`std::make_shared()`은 가변 템플릿으로서 `T`개체의 생성자 인자를 전달받아 `T`개체를 생성하고, `T`개체를 관리하는 `std::shared_ptr`을 생성하여 리턴합니다.
...
`std::make_shared()`를 이용하면 좀더 메모리 효율적으로 `std::shared_ptr`을 사용할 수 있습니다.
...
또한 코드가 간결해지는 효과도 있습니다. `std::shared_ptr<T>{new T{10}}` 보다는 `std::make_shared<T>(10)`가 간결합니다.
...
항상 `std::shared_ptr`로만 생성할 수 있도록 [생성자 접근 차단](https://tango1202.github.io/classic-cpp-oop/classic-cpp-oop-constructors/#%EC%83%9D%EC%84%B1%EC%9E%90-%EC%A0%91%EA%B7%BC-%EC%B0%A8%EB%8B%A8---private-%EC%83%9D%EC%84%B1%EC%9E%90)을 하고 `Create()`함수를 제공한다고 합시다.
...
이제, `T{}` 개체를 공유하는 다른 `std::shared_ptr`을 만드려면, 다음처럼 하면 됩니다.
...
그런데, 상기 방법은 항상 `std::shared_ptr` 형태로 알고 있어야 합니다. 

여러개의 함수에서 사용할때 항상 `std::shared_ptr`로 인수를 전달하는 건 부담이라, 개체 포인터나 개체 참조자로 전달했다가 `std::shared_ptr`을 만들어야 하는 경우가 오면 낭패입니다. 

만약 관리하는 개체로부터 `std::shared_ptr`을 만들기 위해 다음과 같이 작성한다면, `b`가 `a`의 제어 블록을 공유하지 않고 새로운 제어 블록을 만들어 관리하기 때문에 추후 소유권 분쟁이 발생합니다.
...
이럴때 `std::enable_shared_from_this`를 이용하면,  `std::shared_ptr`이 관리하는 개체로부터 `std::shared_ptr`을 생성할 수 있습니다. 다만, `std::shared_ptr`이 미리 만들어져 있어야 합니다.

1. `std::enable_shared_from_this` 를 `public`상속합니다.(`protected`나 `private`상속은 동작하지 않습니다.)
...
`std::shared_ptr`을 이용하면 개체의 복사 생성이나 대입 연산시 포인터를 공유하는 방식으로 소유권 분생을 해결할 수 있습니다.

다음 코드는,

1. `std::shared_ptr`을 멤버 변수로 사용합니다.
2. `new`로 생성된 `Data`를 받기 위해 생성자에서 `std::unique_ptr`을 인자로 사용합니다.
3. 복사 생성자에서 `other`개체의 각 멤버의 복사 생성자를 호출합니다.
4. `std::shared_ptr` 에서 참조 카운트가 0이 되면 소멸시킬 것이므로 소멸자에서는 별다른 작업을 하지 않습니다.
5. 대입 연산자에서는 각 멤버를 대입합니다. 
   
    이때 `std::shared_ptr`의 대입 연산자는 `noexcept`로 선언되어 있어서 예외를 발생하지 않습니다. 포인터 복사와 참조 카운트 변경만 있으니까 예외 발생할게 없는거죠.
...
`std::shared_ptr`은 관리하는 개체를 메모리에 할당하는 `allocator`를 사용자 정의 할 수 있습니다.
...
# std::shared_ptr 별칭 생성자

`std::shared_ptr`은 별칭 생성자(Aliasing Constructor)라는 좀 특별한 생성자를 가지고 있습니다. 이 생성자는 소유권 대상인 개체와 접근 대상 개체가 다른 경우에 사용합니다.
...
2. `MainData` 개체가 `std::shared_ptr`로 관리될 때,
3. 임의의 함수 `f()` 가 `std::shared_ptr<SubData>`를 인자로 사용하는 경우

`MainData` 가 애초에 `std::shared_ptr<SubData>`를 멤버 변수로 가지면 되는 일입니다만, 상황에 따라 그러지 못할 수도 있으깐요.

다음 코드는 별칭 생성자의 사용 예입니다.

1. `MainData`에서 `m_SubData`를 가지고 있으며,
2. 별칭 생성자로 `std::shared_ptr<SubData> subData` 개체를 만듭니다.
...
`std::shared_ptr`의 대소 비교 연산자는 관리하는 개체의 주소를 비교합니다. 보통은 소유권을 가진 개체와 관리하는 개체가 같으므로 상관 없습니다만, 별칭 생성자를 이용하면 이둘이 서로 다르므로, 소유권을 가진 개체로 비교 하고 싶을 때는 `owner_before()`를 사용합니다.
...
`std::shared_ptr` 형변환 할때

```cpp 
dynamic_cast<std::shared_ptr<Base>>(derived);
```

를 사용하면, 내부 관리 개체를 형변환 하는 것이 아니라 `std::shared_ptr`자체를 형변환 하는 것이므로 동작하지 않습니다.

이를 위해 다음과 같은 형변환 함수들을 제공합니다.

|항목|내용|
|--|--|
|`std::const_pointer_cast()`|상수성만 변환|
|`std::static_pointer_cast()`|타입 유사성을 지키며 변환|
|`std::dynamic_pointer_cast()`|타입 유사성을 지키며 변환.<br/>[Runtime Type Info(RTTI)](https://tango1202.github.io/classic-cpp-oop/classic-cpp-oop-inheritance/#runtime-type-infortti%EC%99%80-%ED%98%95%EB%B3%80%ED%99%98)가 있는 개체([가상 함수](https://tango1202.github.io/classic-cpp-oop/classic-cpp-oop-member-function/#%EA%B0%80%EC%83%81-%ED%95%A8%EC%88%98)가 있는 개체)만 가능.|
|`std::reinterpret_pointer_cast()`|상속관계를 무시하고 변환.<br/>정수를 포인터로 변환.|
...
1. `std::shared_ptr`로 부모 포인터를 관리합니다.
2. `std::vector<std::shared_ptr<Node>>` 로 자식을 관리합니다.
...
그래서 `m_Parent`를 `std::shared_ptr`이 아닌 소유권이 없는 일반 포인터로 만들 수도 있지만, `std::weak_ptr`로 해결하는게 예외에 좀더 안전합니다.

# std::weak_ptr

`std::weak_ptr`은 `std::shared_ptr`로부터 생성되며, `std::shared_ptr`과 개체는 공유하지만, 소유권은 공유하지 않는 포인터 입니다. 즉, 참조 카운트를 증가/감소 하지 않고, 제어 블록에서 별도의 `std::weak_ptr` 카운트를 증가/감소 시키기만 할 뿐, 개체를 `delete` 하지 않습니다.(개체는 `std::shared_ptr`에서 `delete`합니다.)

상호 참조에서 `m_Parent`를 `std::weak_ptr`로 수정해 보면,
...
`root` 의 참조 카운트는 1개여서 `root`소멸시 `Node`를 소멸시키며, `std::vector`가 소멸되고, `std::vector`에 추가됐던 자식 `Node`들이 소멸되어, 생성된 `Node`개체들이 모두 정상적으로 소멸됩니다.
...
|항목|내용|
|--|--|
|`constexpr weak_ptr() noexcept;`<br/><br/>`weak_ptr(const std::shared_ptr& other) noexcept;`|`std::shared_ptr`로 부터 생성합니다.|
|`weak_ptr(const weak_ptr& other) noexcept;`|복사 생성합니다.|
|`weak_ptr(const weak_ptr&& other) noexcept;`|이동 생성합니다.|
|`~weak_ptr();`|기존에 관리하던 개체를 해제합니다. `std::shared_ptr`의 `std::weak_ptr` 참조 카운트만 감소시킬 뿐 개체에 대한 `delete`를 수행하지는 않습니다.|
|`weak_ptr& operator =(const weak_ptr& other) noexcept;`<br/><br/>`weak_ptr& operator =(const shared_ptr& other) noexcept;`|`std::weak_ptr`이나 `std::shared_ptr`로 부터 대입받습니다.|
...
|`reset() noexcept;`|기존에 관리하던 개체를 해제합니다. `std::shared_ptr`의 `std::weak_ptr` 참조 카운트만 감소시킬 뿐 개체에 대한 `delete`를 수행하지는 않습니다.|
|`use_count() const noexcept;`|`std::shared_ptr`의 참조 카운트를 리턴합니다.|
...
`std::weak_ptr`는 `std::shared_ptr`이나 `std::weak_ptr`로 부터 생성되며, 관리하는 개체에 접근하려면, `lock()`함수를 이용하여 `std::shared_ptr`을 임시로 생성한 뒤, 이를 사용해야 합니다.
