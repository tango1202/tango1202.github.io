...
`double` 을 사용하는 `A`, `B`, `C`, `D` 4개의 클래스를 더하기를 할때, `A`, `B`는 올림을 하고, `C`, `D`는 내림을 해야 하다고 합시다.
...
`A`, `B`, `C`, `D` 사용자는 각 클래스의 올림과 내림 규칙을 숙지하고, 실수 없이 `AddCeil()`과 `AddFoor()`를 호출해야 합니다. 잘못 사용할 확률이 참 높죠.
...
상기 코드에서 타입을 처리하는 특성만 따로 뽑아내어 `AddTraits` 를 만들 수 있습니다. 타입 처리 방식이 응집되어 유지보수가 좀더 간결해 질 수 있습니다.
