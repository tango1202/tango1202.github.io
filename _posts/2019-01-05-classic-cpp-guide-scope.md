...
정의한 곳과 사용하는 곳이 가깝게 있는 것이 좋습니다. 그래서 함수 앞 부분에 변수 정의를 몰아서 해두는 건 좋지 않습니다. 사용하지도 않는데 미리 비용을 지불하지 마세요.([제로 오버헤드 원칙](https://tango1202.github.io/principle/principle-zero-overhead/)) 쓸변없는 비용 지불은 둘째치고, 나중에 위아래 스크롤 하시기 바빠질 수도 있습니다.(변수명이 뭐였지? 자료형이 뭐였지? 그냥 헝가리언 표기를 할까? 언제 초기화 됐지? 언제 값이 바뀌지?) 
