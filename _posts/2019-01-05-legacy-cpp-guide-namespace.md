...
`using namespace MyLib::Windows;` 와 같이 [using 지시문](https://tango1202.github.io/legacy-cpp-guide/legacy-cpp-guide-namespace/#using-%EC%A7%80%EC%8B%9C%EB%AC%B8)을 사용하여 `YourLib`에서 `Windows`용 `f()`를 호출하도록 했는데요, 이를 `using namespace MyLib::Linux;`로 변경하면 `YourLib`에서 사용하는 모든 `f()`함수를 `Linux`용 `f()`함수로 한방에 변경할 수 있습니다.
