---
layout: single
title: "#6. [모던 C++] (C++11~) 무제한 공용체(unrestricted Union)"
...
다음에서 `MyUnion`은 non-trivial 타입인 `A`와 `B`를 멤버로 사용하고. 두 개체중 크기가 큰 개체의 크기 만큼 메모리를 할당합니다.
...
// A와 B는 non-trival 입니다.
EXPECT_TRUE(std::is_trivial<A>::value == false);
EXPECT_TRUE(std::is_trivial<B>::value == false);  

// MyUnion 은 A, B중 크기가 큰 개체만큼 메모리를 할당합니다.
EXPECT_TRUE(sizeof(A) < sizeof(B) && sizeof(B) == sizeof(MyUnion));

// A, B의 생성자와 소멸자가 호출되지 않습니다.
MyUnion obj;
```

상기 코드를 실행하면, `A`, `B`의 생성자와 소멸자는 호출되지 않고, `MyUnion`만 생성자와 소멸자가 호출됩니다.
