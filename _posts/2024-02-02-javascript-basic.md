...
선언하지 않은 변수가 사용되었을때 변수 선언이 유효 범위의 시작점에 있는 것처럼 이동시켜 처리하며, 이를 변수 호이스팅이라 합니다. 단, 초기값이 대입되지 않은 것처럼 선언되어 값이 할당되기 전까지는 `undefined`입니다.
...
`var`의 유연함과 유효 범위 문제는 대규모 프로젝트를 진행할 때 찾기 힘든 오류를 유발합니다. 따라서 ECMAScript6 부터 `let`과 `const`를 지원합니다.
...
console.log('undefinedVal == null도 true가 되버립니다.', undefinedVal == null); // true. undefined인데, null로 검사해도 true 입니다.
```
...
delete user.name;
console.log("속성 삭제 후 user.name === undefined", user.name === undefined); // name 속성이 없어졌습니다.
```
...
# 개체 생성자 함수의 메서드 중복 생성

하지만 생성자 함수는 코딩 편의성은 주지만, 상기는 사실 다음과 유사하게 함수를 메서드로 사용할 경우 메서드를 중복 생성합니다. 쓸데없이 메모리를 차지하게 되니 메서드는 [prototype](??)에 선언하는게 좋습니다. 

```javascript
const user1 = {
    name: 'Kim',
    number: '123-4567',
    getName: function() {return this.name;} // 메서드 선언이 중복됩니다.
};
const user2 = {
    name: 'Lee',
    number: '111-2222',
