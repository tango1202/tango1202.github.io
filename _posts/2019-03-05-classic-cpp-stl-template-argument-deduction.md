...
템플릿의 인자 타입이 정확히 일치하지 않으면 추론할 수 없습니다. 하기에서 `A<10>`에서 `10`은 `int` 타입이기 때문에 `short`를 사용하는 `f()` 함수로 전달할 수 없습니다. 다만, `f<10>(a)`와 같이 호출하여, `short` 타입 `10`을 전달하여 사용할 수 있습니다.  
...
인자가 템플릿 타입이라면, 더 특수화된 버전을 선택합니다.
