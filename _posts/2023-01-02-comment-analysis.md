...
이런 분 계신가요? 주석이 오히려 눈을 어지럽히나요? 우리는 왜 코드를 봐야 할까요?

함수가 있다면 함수선언만 보면 될 것을 왜 코드를 보는 걸까요? 왜 함수의 실제 구현 코드를 봐야 하고, 함수가 하는 짓을 일일이 추적하며 봐야 할까요?
...
6. 코드도 주석도 리팩토링 합니다.
    ...
    다만 단점은 있습니다. 누군가가 `SetAngle` 함수의 구현해서 degree 인자를 `Radian`으로 처리하게 바꿔놓고 주석이나 인수명을 리팩토링을 하지 않았다면, 사람들은 0~360의 값을 전달하고 오류를 경험하게 될 것입니다. 
