...
템플릿은 인스턴스화 될때 코드를 생성합니다. 따라서, 템플릿 정의 시점에는 멤버 함수가 불완전해도 컴파일이 됩니다. 멤버 변수가 사용되지 않는한, 꼭 정의할 필요는 없습니다.
...
선언과 정의를 분리할 때, 바깥쪽 템플릿과 안쪽 템플릿이 있다면, 2개의 인자 집합이 필요합니다.
...
일반 멤버 변수 함수와 동일하게 템플릿 멤버 함수도 오버로딩 할 수 있습니다. 단, 인자 타입이 동일하면, 일반 멤버 함수가 호출됩니다.
