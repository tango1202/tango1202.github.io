...
> **모던 C++**
> * `unique_ptr`은 소유권 이전용 스마트 포인터입니다. 기존 `auto_ptr`을 대체합니다. `auto_ptr`은 `delete[]` 미지원, `lvalue` 대입 연산시 소유권을 이전하는 이동 동작을 하는 등의 사유로 deprecate 되었습니다.([unique_ptr](https://tango1202.github.io/mordern-cpp-stl/mordern-cpp-stl-unique_ptr/) 참고)
...
C++11 부터는 deprecate되어 더이상 사용하시면 안되고, 다른 스마트 포인터를 사용해야 합니다.([std::unique_ptr](https://tango1202.github.io/mordern-cpp/mordern-cpp-unique_ptr/) 참고)
