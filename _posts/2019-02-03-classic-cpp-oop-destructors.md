...
1. 상속 전용 기반 클래스
    ...
    이런 경우라면 `protected` 생성자를 사용하는게 좋습니다.([상속 전용 기반 클래스 - protected 생성자](https://tango1202.github.io/classic-cpp-oop/classic-cpp-oop-constructors/#%EC%83%81%EC%86%8D-%EC%A0%84%EC%9A%A9-%EA%B8%B0%EB%B0%98-%ED%81%B4%EB%9E%98%EC%8A%A4---protected-%EC%83%9D%EC%84%B1%EC%9E%90) 참고)

    * `protected` Non-Virtual 소멸자인 경우는 상관없지만 `public` Virtual 소멸자는 순가상 함수가 없으면 인스턴스를 생성할 수 있습니다.
  
    * 순가상 소멸자는 별도의 구현 정의를 따로 만들어야 하기 때문에 번거롭습니다.(특히 함수 내부의 로컬 클래스로 정의할때는 소멸자 정의를 할 방법이 없습니다.)
