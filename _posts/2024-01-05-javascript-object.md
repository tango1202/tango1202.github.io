...
상기 그림을 보면, `name`이나 `number`같은 데이터를 저장한 속성은 [개체](https://tango1202.github.io/javascript/javascript-object/#%EA%B0%9C%EC%B2%B4)마다 따로 존재하는게 당연해 보이는데, `getName()`과 같은 메서드까지 따로 존재하는 것은 좀 낭비죠.
