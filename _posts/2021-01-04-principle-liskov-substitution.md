...
이 경우엔 부모가 잘못한 것이라 생각합니다. 모든 `Shape`이 `width`와 `height`를 가진다는 잘못된 가정을 한겁니다. 그래서 자식은 어쩔수 없이 `SetWidth()`와 `SetHeight()`를 재구현할 수 밖에 없었고요.
...
2. 계약에 의한 코드 구현만 준수하면 되므로 구현 편의성이 향상됩니다.
3. 계약에 의한 코드 구현으로 시스템 안정성이 향상됩니다.
