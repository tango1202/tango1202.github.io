...
1. [예외를 탐지](??)하지 않으면, 예외는 상위 개체에 그대로 [전파](??)되고, 
2. [예외를 탐지](??)하면, 더이상 예외는 상위 개체에 [전파](??)되지 않는데요, 이를 다시 [재전파](??)하려면 `throw;`를 사용합니다.
...
다음 코드에서 `f()`에서 [동적 예외 사양](https://tango1202.github.io/classic-cpp-exception/classic-cpp-exception-mechanism/#%EB%8F%99%EC%A0%81-%EC%98%88%EC%99%B8-%EC%82%AC%EC%96%91)에 없는 [예외가 발생](https://tango1202.github.io/classic-cpp-exception/classic-cpp-exception-mechanism/#%EC%98%88%EC%99%B8-%EB%B0%9C%EC%83%9D%EA%B3%BC-%ED%83%90%EC%A7%80try-catch-throw)했을때,
