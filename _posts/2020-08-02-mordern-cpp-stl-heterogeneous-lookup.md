---
layout: single
title: "#29. [모던 C++ STL] (C++14~) 연관 컨테이너의 이종 탐색"
...
> * (C++14~) 연관 컨테이너의 이종 탐색을 지원하여 서로 다른 타입이더라도 탐색이 가능합니다. 

# 개요

기존에는 `map`과 `set`등의 연관 컨테이너에서 키와 동일한 타입으로만 탐색이 가능했습니다.

다음 예에서는 `Lee`의 데이터를 탐색하고 싶어 `Data{"Lee", 10}`와 같이 키와 동일한 `Data` 타입 개체를 만들어 `find()`함수를 호출합니다.
...
C++14 부터는 연관 컨테이너의 이종 탐색을 지원하여 서로 다른 타입이더라도 탐색이 가능합니다.

1. 이종 타입에 대해 비교할 수 있게 정적 비교 함수를 제공합니다.
2. 컨테이너 선언시 `find()` 에서 키 타입외에 다른 것을 사용하려면 `std::less<>`를 사용합니다.
