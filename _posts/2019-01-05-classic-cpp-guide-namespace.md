...
이제 `My` 네임스페이스에 `std::swap()`보다 성능이 좋은 `swap()`함수를 만들었다 칩시다. 새로운 `swap()`함수를 사용하도록 리팩토링 해야 하는데요, 같은 네임스페이스에 있는 함수라 그냥 `std::`만 삭제하면 되죠. 혹시 1,000군데 사용했다면, 찾기/바꾸기로 하면 되고요. 그런데, `std::`버전이 성능이 더 좋아 되돌린다면, 또 찾기/바꾸기를 해야 겠군요. 어딘지 좀 미련한 느낌입니다.
