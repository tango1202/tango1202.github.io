...
코드(텍스트) : read only 프로그램 코드. 제어문, 상수, 함수, 매크로 상수
데이터 : 전역 변수 정적 변수
  BSS(Block Started by Single) : 읽기 / 쓰기 초기화 안된 혹은 0으로 초기화된 전역 변수 또는 정적 변수
  rodata(read only data) : 읽기 문자열 상수.
  data : 읽기 / 쓰기 0 이외 값으로 초기화된 전역, 정적 변수
힙 new 등으로 생성한 동적 메모리 명시적으로 소멸시켜야 함
스택 인자, 지역 변수, 리턴값. 자동 소멸 
