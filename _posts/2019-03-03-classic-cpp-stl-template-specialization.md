...
> * 템플릿 함수는 오버로딩 함수와 특수화 함수의 정의 순서를 지켜라.
...
템플릿 함수 오버로딩 시에는 순서에 주의해야 합니다. 

1. 주 템플릿 함수 정의
2. 템플릿 함수의 오버로딩 버전 정의
3. 템플릿 함수의 특수화 정의

만약 상기 순서대로 정의하지 않으면, #2와 #3의 순서를 변경하면 `int*` 타입으로 실행시,

1. 가장 마지막에 정의된 오버로드 버전인 #2가 호출되고,
2. 명시적으로 호출하면 템플릿 특수화 버전인 #3이 호출됩니다.
...
template<> 
int f(int*) {return 3;} //#3. 템플릿 함수 특수화 
...
안타깝게도 정의 순서에 따라 결과가 달라집니다. 템플릿 함수 특수화는 사용하지 않고 다음처럼 일반 함수로 사용하여 오버로딩 하면, 언뜻 정상이 되는 듯 합니다.
...
```
따라서 반드시 다음 순서에 따라 템플릿 함수를 정의해야 합니다.
