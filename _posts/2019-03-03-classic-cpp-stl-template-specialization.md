...
다음 코드에서 템플릿 함수 특수화 버전인 #3을 #1과 #2 사이에 기재 했습니다. #3 입장에서는 템플릿 함수가 #1 밖에 없으므로, #3은 #1의 특수화 버전입니다.
...
이번에는 템플릿 함수 특수화 버전인 #3 의 순서를 바꿔, #2 다음에 정의하면 결과가 바뀝니다. #3은 #1과 #2중 좀더 적합한 #2의 특수화 버전이 되며, 다음처럼 인스턴스화 후보가 만들어 집니다.
