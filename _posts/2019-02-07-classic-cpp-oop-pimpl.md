...
다음 예제는  [멤버 변수가 2개 이상인 경우 스마트 포인터와 복사 대입 연산자와의 호환성](??) 의 클래스 `T`를 PImpl 방식으로 리팩토링 한 예입니다.
...
6. `T::Impl` 정의에서, 복사 생성자는 두고, 복사 대입 연산자는 사용하지 않기에 `private`로 막았습니다.
...
2. `swap`을 이용한 복사 대입 연산자 구현 편의성을 제공합니다만,
