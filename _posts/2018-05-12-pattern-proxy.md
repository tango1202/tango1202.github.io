...
예를들어 문서 편집 프로그램을 생각해 봅시다. 처음 파일을 열면서 몇페이지가 될지 미리 계산을 하고, 그에 따라 스크롤바를 표시합니다. 이때 문서내에 이미지가 있더라도 이미지를 생성할 필요가 없습니다. 화면 너머에 있는 것은 그려질 필요가 없으니까요. 단지 페이지 계산을 위한 이미지의 크기만 알면 되죠.

따라서, 이미지의 생성은 비용이 비싸므로 최대한 미뤄두는게 좋습니다.
...
3. #3: `ImageProxy`는 `Draw()` 시 실제 `Image`를 생성하며, 그전에는 멤버 변수의 값을 이용합니다.
