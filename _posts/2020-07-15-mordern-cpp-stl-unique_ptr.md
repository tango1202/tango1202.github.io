---
layout: single
title: "#15. [모던 C++ STL] (C++11~) 소유권 이전 스마트 포인터(unique_ptr), (C++14~) make_unique"
...
`make_unique()`은 가변 템플릿으로서 `T`개체의 생성자 인자를 전달받아 `T`개체를 생성하고, `T`개체를 관리하는 `unique_ptr`을 생성하여 리턴합니다.
